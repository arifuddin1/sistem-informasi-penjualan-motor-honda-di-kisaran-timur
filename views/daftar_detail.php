<div class="container">
    <div class="row">
        <div class="col-xs-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title">Informasi Detail Data Penjualan</h3>
                </div>
                <div class="panel-body">
                    <!--Menampilkan data detail daftar nikah-->
                    <?php
                    $sql = "SELECT *FROM tbl_jual WHERE idjual ='" . $_GET ['id'] . "'";
                    //proses query ke database
                    $query = mysqli_query($koneksi, $sql) or die("SQL Detail error");
                    //Merubaha data hasil query kedalam bentuk array
                    $data = mysqli_fetch_array($query);
                    ?>   

                    <!--dalam tabel--->
                    <table class="table table-bordered table-striped table-hover"> 
                        <tr>
                            <td width="200">Id Penjualan</td> <td><?= $data['idjual'] ?></td>
                        </tr>
                        <tr>
                            <td>No Mesin</td> <td><?= $data['nomesin'] ?></td>
                        </tr>
                        <tr>
                            <td>Jenis Motor</td> <td><?= $data['jenismotor'] ?></td>
                        </tr>
						<tr>
                            <td>Tanggal Pembelian</td> <td><?= $data['tanggalpembelian'] ?></td>
                        </tr>
                        <tr>
                            <td>Keterangan</td> <td><?= $data['keterangan'] ?></td>
                        </tr>
                       
                    </table>
				
                </div> <!--end panel-body-->
                <!--panel footer--> 
                <div class="panel-footer">
                    <a href="?page=daftar&actions=tampil" class="btn btn-success btn-sm">
                        Kembali ke Data Penjualan </a>

                </div>
                <!--end panel footer-->
            </div>

        </div>
    </div>
</div>

